...
public final class ByteCache {
    ...
    public final void release(ByteArray entry, boolean shouldZero) {
        if (_cache) {
            ...
            if (entry.getData().length != _entrySize) {
                Log log = OneMFiveAppContext.getInstance().logManager().getLog(ByteCache.class);
    ...
    private class Cleanup implements TimerEvent {
        ...
        public void timeReached() {
            OneMFiveAppContext.getInstance().statManager().addRateData("byteCache.memory." + _entrySize, _entrySize * _available.size(), 0);
