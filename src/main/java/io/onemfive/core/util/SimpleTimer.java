...
public class SimpleTimer {
    ...
    public static SimpleTimer getInstance() {
        return OneMFiveAppContext.getInstance().simpleTimer();
    }
     ...
     */
    public SimpleTimer(OneMFiveAppContext context) {
     ...
     *  Others should use context.simpleTimer2() instead
     */
    protected SimpleTimer(OneMFiveAppContext context, String name) {
     ...
     *  @since 0.9
     */
    protected SimpleTimer(OneMFiveAppContext context, String name, boolean prestartAllThreads) {
        _name = name;
    ...
    private static class CustomScheduledThreadPoolExecutor extends ScheduledThreadPoolExecutor {
        ...
        protected void afterExecute(Runnable r, Throwable t) {
            ...
            if (t != null) { // shoudn't happen, caught in RunnableEvent.run()
                Log log = OneMFiveAppContext.getInstance().logManager().getLog(SimpleTimer.class);
...
//           }
     ...
     * @since 0.9.20
     */
    public void addEvent(final TimerEvent event, final long timeoutMs) {
        if (event == null)
     ...
     */
    public void addPeriodicEvent(final TimerEvent event, final long timeoutMs) {
     ...
     * @throws IllegalArgumentException if timeoutMs less than 5000
     */
    public void addPeriodicEvent(final TimerEvent event, final long delay,  final long timeoutMs) {
    ...
    public static abstract class TimedEvent implements Runnable {
        ...
        private final SimpleTimer _pool;
        private int _fuzz;
        ...
        public TimedEvent(SimpleTimer pool) {
            _pool = pool;
            _fuzz = DEFAULT_FUZZ;
            _log = OneMFiveAppContext.getInstance().logManager().getLog(SimpleTimer.class);
            _state = TimedEventState.IDLE;
        }

        /** automatically schedules, don't use this one if you have other things to do first */
        public TimedEvent(SimpleTimer pool, long timeoutMs) {
            this(pool);
            schedule(timeoutMs);
        }
    ...
    private static abstract class PeriodicTimedEvent extends TimedEvent {
         ...
         * @throws IllegalArgumentException if timeoutMs less than 5000
         */
        public PeriodicTimedEvent(SimpleTimer pool, long delay, long timeoutMs) {
            super(pool, delay);
            if (timeoutMs < 5000)
