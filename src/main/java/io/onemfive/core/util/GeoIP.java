...
import io.onemfive.core.util.geoip.InvalidDatabaseException;
import io.onemfive.core.util.geoip.LookupService;
...
public class GeoIP {
     ...
     */
    public GeoIP(OneMFiveAppContext context) {
        _context = context;
 ...
 ******/
    ...
    private String[] readGeoIPFile(Long[] search) {
        ...
        if (_log.shouldLog(Log.INFO)) {
            _log.info("GeoIP processing finished, time: " + (new Date().getTime() - start));
        }
        return rv;
    }
    ...
    private void updateOurCountry() {
        OneMFiveAppContext ctx = _context;
//        String oldCountry = ctx.getConfigSetting(PROP_IP_COUNTRY);
...
//            ctx.conscious().saveConfig(PROP_IP_COUNTRY, country);
//            if (ctx.commSystem().isInBadCountry() && ctx.getProperty(Conscious.PROP_HIDDEN_HIDDEN) == null) {
...
//                ctx.conscious().rebuildConsciousInfo();
//            }
//        }
        /****/
    }
